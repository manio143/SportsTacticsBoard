...
namespace SportsTacticsBoard.PlayingSurfaceTypes
{
  ...
  /// </summary>
  class Futsal : SportsTacticsBoard.IPlayingSurfaceType
  {
    ...
    public Collection<FieldObject> StandardObjects
    {
      get {
        ...
        // Add the referees
        fieldObjects.Add(new FieldObjects.Referee("R1", "Referee_Futsal_1", Length * 3.0F / 4.0F, Width + (playerSize * 2.05F), playerSize));
        fieldObjects.Add(new FieldObjects.Referee("R2", "Referee_Futsal_2", Length / 4F, -(playerSize * 2.05F), playerSize));
    ...
    public void DrawMarkings(Graphics graphics)
    {
      ...
      Pen additionalMarkPen = new Pen(Color.White, 0.06F);
